...
public class Rosette_Addition2 implements PlugInFilter {
...
float[] px =(float[]) stack.getPixels(1);



IJ.showStatus("Reaction Diffusion");

int nSteps = 40; // maximum number of steps
double randomFeed = 0.3;
...
for (int x=0; x<w; x++){
	...
	//if(px[(y*w)+x]>0) B[(y*w)+x] = (px[(y*w)+x])/2 ;
